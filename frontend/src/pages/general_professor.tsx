    ...
    <div className="p-4">
      <div style={{ display: 'flex', gap: 8, alignItems: 'center', flexWrap: 'wrap' }}>
        ...
        <input
          ...
          style={{ padding: 6, flex: '1 1 360px', minWidth: 240 }}
        />
      ...
      {data && (
        <div style={{ marginTop: 16 }}>
          <div style={{ fontWeight: 600 }}>{data.professor ?? (professorName || 'Professor Result')}</div>
          {typeof data.total_students === 'number' && (
            <div style={{ color: '#555', marginTop: 4 }}>Total students: {data.total_students}</div>
          )}
          {typeof data.courses_taught_count === 'number' && (
            <div style={{ color: '#555', marginTop: 4 }}>Courses taught: {data.courses_taught_count}</div>
          )}
          {typeof data.avg_gpa === 'number' && (
            <div style={{ color: '#555', marginTop: 4 }}>Avg GPA: {data.avg_gpa.toFixed(3)}</div>
          )}
          {typeof data.courses_taught === 'string' && data.courses_taught && (
            <div style={{ color: '#555', marginTop: 4 }}>Course list: {data.courses_taught}</div>
          )}
